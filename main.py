...
# ==========================================
# ⚖️ ЮРИДИЧЕСКАЯ БАЗА (ОФЕРТА)
# ==========================================
LEGAL_TEXT = (
    "<b>📜 ПУБЛИЧНАЯ ОФЕРТА И ПРАВИЛА СЕРВИСА</b>\n\n"
    "1. <b>Статус Сервиса:</b> Данный бот является информационным агрегатором (платформой), предоставляющим доступ к базе данных сценариев развлекательного досуга.\n"
    "2. <b>Отказ от ответственности:</b> Сервис НЕ является перевозчиком, таксопарком или транспортной компанией. Услуги перевозки оказываются третьими лицами (Водителями), имеющими соответствующие лицензии.\n"
    "3. <b>Безопасность:</b> Клиент обязуется не требовать от Водителя действий, нарушающих ПДД РФ. Водитель имеет право отказать в оказании услуги «Crazy Menu», если это угрожает безопасности.\n"
    "4. <b>Финансы:</b> Оплата за транспортные услуги производится Водителю. Оплата за доступ к развлекательному контенту производится на основании добровольного согласия Клиента.\n"
    "5. <b>Согласие:</b> Продолжая использование бота, вы подтверждаете, что ознакомлены с данными правилами и не имеете медицинских противопоказаний к громким звукам, эмоциональным всплескам и специфическому юмору."
)
...
def init_db():
    ...
    # 2. Клиенты (с привязкой к водителю)
    cursor.execute("""
...
async def safe_send_message(chat_id, text, reply_markup=None):
    """Безопасная отправка сообщений (игнорирует блокировки)"""
    try:
...
async def rate_ride(callback: types.CallbackQuery, state: FSMContext):
    ...
    if rating > 0: update_order_rating(last_id=0, rating=rating, driver_id=did)
...
async def vip_fio(message: types.Message, state: FSMContext):
    await state.update_data(fio=message.text); await message.answer("🚘 Авто:"); await state.set_state(DriverVipRegistration.waiting_for_car)

@dp.message(DriverVipRegistration.waiting_for_car)
async def vip_car(message: types.Message, state: FSMContext):
    await state.update_data(car=message.text); await message.answer("💳 Реквизиты:"); await state.set_state(DriverVipRegistration.waiting_for_payment_info)

@dp.message(DriverVipRegistration.waiting_for_payment_info)
async def vip_pay(message: types.Message, state: FSMContext):
    await state.update_data(pay=message.text); await message.answer("🔑 Код-пароль:"); await state.set_state(DriverVipRegistration.waiting_for_code)
