...
async def cab(message: types.Message, state: FSMContext):
    ...
    conn = sqlite3.connect(DB_PATH)
    history = conn.execute("SELECT service_name, price, rating FROM order_history WHERE driver_id=? ORDER BY id DESC LIMIT 5", (message.from_user.id,)).fetchall()
    conn.close()
...
async def edit_driver_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Ред: {info[7]} ({info[10]}%)", reply_markup=kb)
...
async def bill_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Как выставить счет?", reply_markup=kb)
    await callback.answer()

@dp.callback_query(F.data == "bill_send_def")
async def bill_def(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    did = data['target_did']
    boss = get_driver_info(OWNER_ID)
    info = get_driver_info(did)
    ...
    await callback.message.edit_text("✅ Счет отправлен.")
    await state.clear()

@dp.callback_query(F.data == "bill_send_cust")
async def bill_cust(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text("Введите текст счета:")
    await state.set_state(AdminBilling.waiting_for_custom_req)

@dp.message(AdminBilling.waiting_for_custom_req)
async def bill_send_custom_txt(message: types.Message, state: FSMContext):
    data = await state.get_data()
    did = data['target_did']
...
async def edit_vals(callback: types.CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    did = int(parts[2])
    field = "balance" if parts[1] == "bal" else "commission"
    await state.update_data(did=did, fld=field)
    await callback.message.answer(f"Новое значение для {field}:")
    await state.set_state(AdminEditDriver.waiting_for_new_value)
    await callback.answer()

@dp.message(AdminEditDriver.waiting_for_new_value)
async def save_vals(message: types.Message, state: FSMContext):
...
async def block_driver(callback: types.CallbackQuery, state: FSMContext):
    did = int(callback.data.split("_")[2])
    info = get_driver_info(did)
